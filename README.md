...
# 2022.1.13
今天来学习一些简单点的，昨天的还是看的我有点蒙b

首先是final关键字的用法,就是用了他就不能再改变了。
类用了他修饰的话，就不能再被继承了，方法用了他，就不能再被覆盖重写了  

对于内部类，其中最重要的就是匿名内部类，看day08-code里面的demo05就知道具体咋用了
下面对格式"new 接口名称() {}"进行解析  
...
+ 匿名对象，在调用方法的时候，只能调用唯一一次，如果希望同一个对象调用多次方法，那么必须给对象取个名字
+ 假设匿名内部类里面有很多方法，你用的还是一个匿名对象，那么你使用的时候也只能从这些方法里面选一个
+ 匿名内部类是省略了实现类/子类的名称，到那时匿名对象是省略了对象mc，强调：匿名内部类和匿名对象不是一回事

成员变量不光能用基本类型，还能用自己定义的类

同时接口也可以作为成员变量的类型

在day08-code当中的demo07当中
有一个类Hero，一个接口Skill，一个接口实现类SkillImpl
Hero使用了Skill类作为成员变量，意思就是使用技能，那么当不用接口实现的方法时，我们使用匿名对象和匿名内部类的方法来帮助Hero设置Skill
那么我们就需要实现接口当中的方法下面是具体的写法
```
hero.setSkill(new Skill(){
     @Override
     public void use(){
       System.out.ptintfln("我是匿名内部类")
     }
 })；
```
